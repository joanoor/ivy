...
>(
  ...
  >(
    ...
    try {
      let {
        result,
        columns,
      }: { result: Struct2[]; columns: ResultColumnsData[] } =
        (await requestPromiseFunc(requestOpt, {
          ...opt, // RequestOptions
        })) as unknown as {
          result: Struct2[]
      ...
      if (!isDownloadXlsx) {
        // @ts-ignore
        tdata.tableData = result as UnwrapRef<Struct2[]>
  ...
  const onGetAllSelectedRows = () => {
    tdata.allSelectedData =
      // @ts-ignore
      (tableRef.value?.tableRef2 || tableRef.value).getSelectionRows() as UnwrapRefSimple<Struct>[]
  }

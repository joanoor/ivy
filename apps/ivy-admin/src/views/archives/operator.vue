<template>
  <div bg-white pl-5 pr-5 pb-5>
    ...
    <ButtonList mb-5>
      <template #left>
        ...
        <el-button
          ...
          size="default"
          mr-2
          @click="downloadTemplate('/运营商导入模板.xlsx')"
        >
          模板下载
        </el-button>
        <el-upload
    ...
    <SelectionTable
      ...
      @edit-record="handleEditOperator"
    ...
    <el-form :model="form2" :rules="rules2" ref="formRef2" label-width="100px">
      ...
      >
        ...
        <template v-else-if="col.component === 'radio'">
          <el-form-item prop="operatorStatus">
            <el-radio-group v-model="form2.operatorStatus">
...
import { ResultOperatorStruct } from '@/api/dossier/model'
import { OperatorDeleteRow } from '@/api/dossier/index'
import { submitForm } from '@/utils/formAndRules/form'
import { getAreaPath, downloadTemplate } from '@/utils/index'

import { addOperatormanage, OperatorEditRow } from '@/api/dossier'
...
const {
  ...
  formRef,
  handleResetForm,
} = useForm(['operatorNo', 'operatorName'])
...
const {
  ...
  requestUrl: API.OPERATOR_MANAGER,
  queryParams: { data: {} },
  customColumns: [
    {
      name: 'createdTime',
  ...
  expectOrderColumnNames: ['operatorName'],

  //忽略form表中的某列，useQueryTable中引用
...
const {
  ...
  columns,
  {
    expectOmitedColumnNames: ['id'],
    customDictionary: {
      CONNECTORTYPE: useGlobal.dicts['supervision_unit'],
    },
  }
)
...
const handleSubmit = submitForm(async (valid?: boolean) => {
  if (valid) {
    // form2.value.address=form2.value.address.
    ...
    if (dialogTitle.value.includes('新增')) {
      await addOperatormanage(form2.value)
      dialogVisiable.value = false
      fetchTableList()
    } else if (dialogTitle.value.includes('修改')) {
      OperatorEditRow(form2.value).then(() => {
...
const handleDeleteRecord = async (row: ResultOperatorStruct) => {
  await OperatorDeleteRow(row.id)
  ElMessage.success({
    message: '删除成功',
  })
  fetchTableList()
...
const handleAddOperator = () => {
  dialogTitle.value = '新增运营商'
  dialogVisiable.value = true
}

const handleEditOperator = (row: ResultOperatorStruct) => {
  dialogTitle.value = '修改运营商'
  dialogVisiable.value = true
  onEchoForm(row)
}
...
const handleBatchDelete = async () => {
  ...
  if (allSelectedData.value.length > 0) {
    await OperatorBatchDelete(ids.value)
    fetchTableList()
  } else {
